...
import case_readers.util as ut
...
class CaseDataReader(DataReader):
    ...
    def read_jet_constituents_from_file(self, file):
        ...
        j1_constituents_pt_eta_phi = np.array(file.get(self.jet1_constituents_key))[:,:,:] # (batch, 100, 3)
        j2_constituents_pt_eta_phi = np.array(file.get(self.jet2_constituents_key))[:,:,:] # (batch, 100, 3)
        ...
        j1_constituents[:,:,0] = j1_constituents_pt_eta_phi_sorted[:,:,0]*np.cos(j1_constituents_pt_eta_phi_sorted[:,:,2]) #px = pt cos(phi)
        j1_constituents[:,:,1] = j1_constituents_pt_eta_phi_sorted[:,:,0]*np.sin(j1_constituents_pt_eta_phi_sorted[:,:,2]) #py = pt sin(phi)
        j1_constituents[:,:,2] = j1_constituents_pt_eta_phi_sorted[:,:,0]*np.sinh(j1_constituents_pt_eta_phi_sorted[:,:,1]) #pz = pt sinh(eta)
        j2_constituents[:,:,0] = j2_constituents_pt_eta_phi_sorted[:,:,0]*np.cos(j2_constituents_pt_eta_phi_sorted[:,:,2]) #px = pt cos(phi)
        j2_constituents[:,:,1] = j2_constituents_pt_eta_phi_sorted[:,:,0]*np.sin(j2_constituents_pt_eta_phi_sorted[:,:,2]) #py = pt sin(phi)
        j2_constituents[:,:,2] = j2_constituents_pt_eta_phi_sorted[:,:,0]*np.sinh(j2_constituents_pt_eta_phi_sorted[:,:,1]) #pz = pt sinh(eta)


        #pt1 = np.where(j1_constituents[:,:,0]!=0,j1_constituents[:,:,0],-1)
        #pt2 = np.where(j2_constituents[:,:,0]!=0,j2_constituents[:,:,0],-1)
        #j1_constituents[:,:,0] = pt1
        #j2_constituents[:,:,0] = pt2

        '''
        eta1 = np.where(j1_constituents[:,:,1]!=0,j1_constituents[:,:,1],-1)
        eta2 = np.where(j2_constituents[:,:,1]!=0,j2_constituents[:,:,1],-1)
        j1_constituents[:,:,1] = eta1
        j2_constituents[:,:,1] = eta2

        phi1 = np.where(j1_constituents[:,:,2]!=0,j1_constituents[:,:,2],-1)
        phi2 = np.where(j2_constituents[:,:,2]!=0,j2_constituents[:,:,2],-1)
        j1_constituents[:,:,2] = phi1
        j2_constituents[:,:,2] = phi2
